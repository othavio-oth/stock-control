...
@router.get("/units/", response_model=List[UnitMeasurementResponse], )
def get_all_units(db: Session = Depends(get_db)):
    return list_unit_measurement(db)

@router.post("/units/", response_model=UnitMeasurementResponse, )
...
@router.put("/units/{unit_id}", response_model=UnitMeasurementResponse, )
