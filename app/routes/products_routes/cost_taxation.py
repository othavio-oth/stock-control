...
@router.get("/cost_taxations/", response_model=List[CostTaxationResponse])
def get_cost_taxations(db: Session = Depends(get_db)):
    return list_cost_taxations(db)

@router.post("/cost_taxations/", response_model=CostTaxationResponse)
...
@router.put("/cost_taxations/{cost_id}", response_model=CostTaxationResponse)
...
@router.delete("/cost_taxations/{cost_id}")
